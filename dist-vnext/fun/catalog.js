...
export const FUN_COMMAND_TOKENS = Object.freeze(FUN_COMMAND_DESCRIPTORS.flatMap((item) => item.tokens));
const byToken = new Map();
for (const item of FUN_COMMAND_DESCRIPTORS) {
    for (const token of item.tokens) {
        if (byToken.has(token))
            throw new Error(`Token duplicado no domínio de brincadeiras: ${token}`);
        byToken.set(token, item);
    }
}
export function findFunCommandDescriptor(command) {
