 ...
 */
export class AtomicDomainDispatchTarget {
    name;
    state;
    tokenSet;
    target;
    constructor(options) {
        ...
        this.tokenSet = new Set(options.tokens
            .map((token) => String(token || '').trim().toLowerCase())
            .filter(Boolean));
        if (!this.name.trim())
            throw new Error('AtomicDomain precisa de nome.');
        if (this.tokenSet.size === 0)
            throw new Error(`AtomicDomain ${this.name} não possui tokens.`);
    }
    async dispatch(command, context) {
        const normalized = String(command || '').trim().toLowerCase();
        if (!normalized || !this.tokenSet.has(normalized))
            return false;
        if (this.state === 'staged')
            return false;
        const handled = await this.target.dispatch(normalized, context);
        if (!handled) {
            throw new Error(`Domínio atômico ${this.name} está ativo, mas recusou o token owned ${normalized}.`);
        }
        return true;
    }
}
//# sourceMappingURL=atomic-domain.js.map