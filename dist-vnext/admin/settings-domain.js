...
function normalizeBlockedCommand(value, prefix) {
    return value
        .trim()
        .toLowerCase()
        .normalize('NFD')
...
async function withGroupState(context, mutator, errorMessage) {
...
async function deleteMessageLimit(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    try {
        const store = new JsonGroupStateStore(context.groupFile, context.groupData);
...
async function setPrefix(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
...
async function toggleExit(context) {
    await withGroupState(context, (state) => {
        const exit = isRecord(state.exit) ? { ...state.exit } : {};
...
async function toggleAntiSticker(context) {
    await withGroupState(context, (state) => {
...
async function addRule(context) {
    ...
    await withGroupState(context, (state) => {
        const rules = Array.isArray(state.rules)
            ? state.rules.filter((item) => typeof item === 'string')
            : [];
        rules.push(q);
        state.rules = rules;
...
async function setMinimumMessage(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    const args = argsOf(context);
...
//# sourceMappingURL=settings-domain.js.map