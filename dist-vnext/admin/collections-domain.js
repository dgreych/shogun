...
function whitelistOf(state) {
    const source = isRecord(state.adminWhitelist) ? state.adminWhitelist : {};
    const output = {};
    for (const [user, value] of Object.entries(source)) {
        if (!isRecord(value) || !Array.isArray(value.antis))
            continue;
        output[user] = {
            antis: value.antis.filter((item) => typeof item === 'string'),
...
function reasonFromQuery(context) {
...
async function warnUser(context) {
    ...
    try {
        ...
        if (current.length >= 3) {
            ...
            await context.reply(`🚫 @${context.getUserName(target)} recebeu 3 advertências e foi banido!\nÚltima advertência: ${reason}`, { mentions: [target] });
            return;
        }
        syncContext(context, state);
        await context.reply(`⚠️ @${context.getUserName(target)} recebeu uma advertência (${current.length}/3).\nMotivo: ${reason}`, { mentions: [target] });
    }
    catch (error) {
...
async function removeWarning(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    const target = context.mentionedUser;
    if (!target) {
    ...
    try {
        ...
        store.write(state);
        syncContext(context, state);
        await context.reply(`✅ Uma advertência foi removida de @${context.getUserName(target)}. Advertências restantes: ${current.length}/3`, { mentions: [target] });
    }
    catch (error) {
...
async function listWarnings(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    try {
...
async function addWhitelist(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    const target = context.mentionedUser;
    if (!target) {
    ...
    try {
        ...
        store.write(state);
        syncContext(context, state);
        await context.reply(`✅ @${context.getUserName(target)} adicionado à whitelist!\n\n*Antis ignorados:*\n${antis.map((anti) => `• ${anti}`).join('\n')}`, { mentions: [target] });
    }
    catch (error) {
...
async function removeWhitelist(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    const target = context.mentionedUser;
    if (!target) {
...
async function listWhitelist(context) {
    if (!context.isGroup) {
...
async function listBlacklist(context) {
    if (!(await ensureAdminAccess(context, adminAccess())))
        return;
    try {
...
async function listModerators(context) {
    if (!context.isGroup) {
        await context.reply('Este comando só funciona em grupos.');
        return;
    }
    try {
        const moderators = Array.isArray(context.groupData.moderators)
            ? context.groupData.moderators.filter((item) => typeof item === 'string')
            : [];
...
//# sourceMappingURL=collections-domain.js.map