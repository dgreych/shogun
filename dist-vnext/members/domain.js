import { MembersGeneratedDomainDispatchTarget, MEMBERS_GENERATED_COMMAND_TOKENS, } from './generated-domain.js';
import { AtomicDomainDispatchTarget, } from '../runtime/atomic-domain.js';
/**
 ...
 */
export class MembersDomainDispatchTarget {
    state;
    tokens = new Set(MEMBERS_GENERATED_COMMAND_TOKENS);
    atomic;
    reportError;
    scopeCircuitOpen = false;
    constructor(options = {}) {
        this.state = options.state ?? 'staged';
        this.reportError = options.reportError ?? ((message, error) => console.error(message, error));
        this.atomic = new AtomicDomainDispatchTarget({
            name: 'members',
            state: this.state,
    ...
    async dispatch(command, context) {
        ...
        if (!normalized || !this.tokens.has(normalized))
            return false;
        if (this.state === 'staged')
            return false;
        if (this.scopeCircuitOpen)
            return false;
        let membersScope;
        try {
            ...
            if (structural)
                this.scopeCircuitOpen = true;
            this.reportError(structural
                ? '[VNEXT/MEMBERS] Falha estrutural antes do handler; circuit breaker de Members aberto e legado preservado.'
                : '[VNEXT/MEMBERS] Falha ao montar scope antes do handler; legado preservado para esta mensagem.', error);
            return false;
        }
