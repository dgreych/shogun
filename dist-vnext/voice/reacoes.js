 ...
 * que reação repetida.
 */
function indiceEstavel(nome, tamanho) {
    let soma = 0;
...
export function temaDoComando(comando) {
    const nome = semAcento(String(comando || ''));
    if (!nome)
        return null;
    for (const [radicais, reacoes] of TEMAS) {
        if (radicais.some((r) => nome.includes(r)))
            return reacoes;
    }
    return null;
}
 ...
 */
export function reacaoDoComando(comando) {
    const nome = semAcento(String(comando || ''));
    if (!nome)
        return NEUTRAS[0];
...
export function reacoesDistintas() {
    return new Set([...TEMAS.flatMap(([, r]) => r), ...NEUTRAS]).size;
}
//# sourceMappingURL=reacoes.js.map