...
export class LinkCheckToolsDomainDispatchTarget {
    ...
    async dispatch(command, context) {
        ...
        if (!LINK_CHECK_TOOLS_NATIVE_COMMAND_TOKENS.includes(normalized))
            return false;
        await this.verifyLink(normalized, context);
        return true;
    }
    async verifyLink(command, context) {
        try {
            ...
            const response = await this.httpGet(`https://api.fishfish.gg/v1/domains/${encodeURIComponent(domain)}`, {
                timeout: 120000,
...
//# sourceMappingURL=link-check-domain.js.map