...
async function timeCommand(context) {
...
async function groupStats(context) {
    if (!context.isGroup) {
    ...
    try {
        ...
        const rawCreation = metadata.creation;
        ...
        const resources = [];
        if (context.groupData.modorpg)
            resources.push('⚔️ Modo RPG');
        if (context.groupData.welcome)
            resources.push('👋 Boas-vindas');
        if (context.groupData.antifake)
            resources.push('🛡️ Anti-fake');
        if (context.groupData.antilink)
            resources.push('🔗 Anti-link');
        if (context.groupData.antilinksoft)
            resources.push('🔗 Anti-link Soft');
        if (context.groupData.antiflood)
            resources.push('🌊 Anti-flood');
...
async function noteCommand(context) {
...
async function notesCommand(context) {
    const notes = context.notes;
    if (!notes) {
        await context.reply('Sistema de notas temporariamente indisponível.');
        return;
...
async function calculatorCommand(context) {
...
async function reminderCommand(context) {
    try {
        const query = context.query.trim();
        if (!query) {
...
async function reminderListCommand(context) {
    try {
        const loadReminders = requiredService(context.loadReminders, 'loadReminders');
        const optimizer = requiredService(context.optimizer, 'optimizer');
        const tzFormat = requiredService(context.tzFormat, 'tzFormat');
...
async function reminderDeleteCommand(context) {
    try {
        ...
        if (!removed)
            throw new Error('Lembrete localizado desapareceu antes da remoção.');
        saveReminders(list);
        optimizer.clearStatic('reminders:all');
...
async function birthdayCommand(context) {
    if (!context.isGroup) {
        await context.reply('⚠️ Este comando só funciona em grupos!');
        return;
    }
