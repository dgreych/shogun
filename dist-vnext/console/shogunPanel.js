...
const SAMURAI_X = 9;
const SAMURAI_Y = 8;
function montarSamurai() {
    const sprite = SAMURAI.map((linha) => [...linha]);
    const faixa = (y, inicio, fim, pixel) => {
        const linha = sprite[y];
        if (!linha)
            return;
        for (let x = Math.max(0, inicio); x <= Math.min(linha.length - 1, fim); x += 1)
            linha[x] = pixel;
    };
    const ponto = (x, y, pixel) => faixa(y, x, x, pixel);
    const linha = (x0, y0, x1, y1, pixel) => {
        let x = x0;
        let y = y0;
...
function montarQuadro() {
    ...
    const topos = Array.from({ length: LARGURA }, (_, x) => {
        const t = x / (LARGURA - 1);
        return Math.round(ALTURA - 5 - t * 3.6 - Math.sin(t * Math.PI) * 2.6 - Math.sin(t * Math.PI * 3) * 0.55);
    });
    for (let x = 0; x < LARGURA; x += 1) {
        if (x < 3 || x > LARGURA - 4)
            continue;
        ...
        for (let y = topo; y <= Math.min(ALTURA - 1, topo + profundidade); y += 1) {
            if (y === topo)
                ...
                tela[y][x] = (x * 7) % 11 === 0 ? 'H' : 'h';
            else if (y === topo + profundidade && (x * 3 + y) % 5 !== 0)
                continue;
            else
                tela[y][x] = (x + y) % 7 === 0 ? 'h' : 'm';
        }
        if ((x % 9 === 2 || x % 13 === 7) && topo > 0)
            tela[topo - 1][x] = 'H';
        if (x % 17 === 5 && topo > 1) {
            tela[topo - 1][x] = 'H';
            if (x + 1 < LARGURA)
 ...
 */
export function renderConnectionPanel(opcoes = { estado: 'aguardando' }) {
...
//# sourceMappingURL=shogunPanel.js.map