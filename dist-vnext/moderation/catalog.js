...
export const MODERATION_COMMAND_TOKENS = Object.freeze(MODERATION_COMMAND_DESCRIPTORS.flatMap((item) => item.tokens));
const byToken = new Map();
for (const item of MODERATION_COMMAND_DESCRIPTORS) {
    for (const token of item.tokens) {
        if (byToken.has(token))
            throw new Error(`Token duplicado no domínio de moderação: ${token}`);
        byToken.set(token, item);
    }
}
export function findModerationCommandDescriptor(command) {
