...
try {
  ...
  const versaoFfmpeg = execSync('ffmpeg -version', { timeout: 10000 }).toString().split('\n')[0];
  await fs.appendFile(__dirname + '/../logs/debug-trigger.log', JSON.stringify({ ts: new Date().toISOString(), marca: 'FFMPEG_CHECK', ok: true, versao: versaoFfmpeg }) + '\n').catch(() => {});
} catch (ffmpegCheckError) {
  await fs.appendFile(__dirname + '/../logs/debug-trigger.log', JSON.stringify({ ts: new Date().toISOString(), marca: 'FFMPEG_CHECK', ok: false, erro: String(ffmpegCheckError && ffmpegCheckError.message || ffmpegCheckError) }) + '\n').catch(() => {});
}

function logCrashToDisk(origem, err, info) {
  const linha = JSON.stringify({
    ts: new Date().toISOString(),
    origem,
    erro: String(err && err.stack || err && err.message || err),
    msgId: info?.key?.id || null
  }) + '\n';
...
async function createGroupMessage(NazunaSock, groupMetadata, participants, settings, isWelcome = true) {
    ...
    let image = null;
    let imageMime = null;
    if (settings.image === 'api') {
    const cardTexto = isWelcome ? 'Bem vindo (a)!' : 'Até logo!';
    const card = await gerarWelcomeCard(
        profilePicUrl,
        bannerName,
        cardTexto,
        settings.background || settings.fundo,
        settings.corMoldura,
        settings.corLinhas,
        false,
        {
        event: isWelcome ? 'join' : 'leave',
    ...
    if (card.ok) {
        ...
        console.error('❌ Erro ao gerar card de entrada/saída', { code: card.code || 'CARD_FAILED' });
    }
    } else if (settings.image !== 'banner') {
    image = { url: settings.image };
    }

    if (image) {
    message.image = image;
    if (imageMime) message.mimetype = imageMime;
...
async function createBotSocket(authDir) {
    ...
    // Extração mínima de texto só pra decidir se a mensagem "parece um
    // comando" antes de avisar o usuário que algo quebrou — não precisa
    // ser exaustiva como o getMessageText de index.js, só o suficiente pra
    ...
    // Antes disso, qualquer erro não tratado em processMessage terminava
    // só em log de servidor — o usuário via a reação de emoji (disparada
    // cedo, fire-and-forget, antes de qualquer processamento real) e
    // depois silêncio absoluto, sem forma de saber se o comando falhou ou
    // ainda estava processando. Ficou impossível de diagnosticar "vários
    // comandos sem resposta" sem um retorno mínimo pro chat de origem.
    ...
    logCrashToDisk('processItem', error, item.message);
    ...
    console.error('❌ Falha ao avisar o usuário sobre erro de comando:', notifyError.message);
    }
    };
