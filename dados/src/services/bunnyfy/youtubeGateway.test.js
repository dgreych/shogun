...
test('cliente YouTube exige HTTPS fora do loopback', () => {
  assert.throws(
    () => createBunnyFyYoutubeClient({
      ...ACTIVE_ENV,
      BUNNYFY_BASE_URL: 'http://api.bunnyfy.test',
      BUNNYFY_ALLOW_INSECURE_HTTP: 'true'
    }),
    error => error instanceof BunnyFyError && error.code === 'BUNNYFY_CONFIG_INVALID'
  );

  const local = createBunnyFyYoutubeClient({
    ...ACTIVE_ENV,
    BUNNYFY_BASE_URL: 'http://127.0.0.1:18080'
  });
  assert.equal(local.baseUrl, 'http://127.0.0.1:18080');
});
...
test('vídeo usa BunnyFy para URL e query resolvida sem reativar downloader legado', async () => {
  const received = [];
  const client = {
  ...
  const metadataAdapter = legacyYoutubeFixture();
  const result = await downloadYoutubeVideoForPlay('Vídeo de teste', {
    env: ACTIVE_ENV,
    legacyYoutube: metadataAdapter,
    clientFactory: () => client,
    quality: '360p'
  });
  assert.equal(result.ok, true);
  ...
  assert.deepEqual(metadataAdapter.calls.map(call => call.method), ['search', 'metadata']);

  const primaryAdapter = legacyYoutubeFixture();
  const unavailable = await downloadYoutubeVideoForPlay('Vídeo de teste', {
    env: { ...ACTIVE_ENV, BUNNYFY_YOUTUBE_MODE: 'primary' },
    legacyYoutube: primaryAdapter,
    clientFactory: () => ({
  ...
  assert.equal(unavailable.code, 'BUNNYFY_UNAVAILABLE');
  assert.deepEqual(primaryAdapter.calls.map(call => call.method), ['search', 'metadata']);
  assert.equal(primaryAdapter.calls.some(call => call.method === 'mp4'), false);
});

test('modo off não constrói cliente nem executa downloader legado', async () => {
  const metadataAdapter = legacyYoutubeFixture();
  const result = await downloadYoutubeAudioForPlay('Faixa desligada', {
  ...
  assert.equal(result.code, 'BUNNYFY_DISABLED');
  assert.deepEqual(metadataAdapter.calls, []);
});

test('primary retorna indisponibilidade em falha transitória sem executar downloader legado', async () => {
  const metadataAdapter = legacyYoutubeFixture();
  ...
  assert.equal(result.ok, false);
  assert.equal(result.code, 'BUNNYFY_UNAVAILABLE');
  assert.deepEqual(metadataAdapter.calls, []);
...
test('modo off por URL também não consulta nem executa downloader legado', async () => {
  const metadataAdapter = legacyYoutubeFixture();
  const result = await downloadYoutubeAudioForPlay(
    'https://youtu.be/legacy00001',
    { env: { BUNNYFY_ENABLED: 'false' }, legacyYoutube: metadataAdapter }
  );
