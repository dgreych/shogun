...
async function processUserMessages(data, nazu = null, ownerNumber = null, personality = 'nazuna', modoAdulto = false) {
  try {
    ...
    for (const msgValidada of mensagensValidadas) {
      ...
      let result;
      try {
        // Aplica a personalidade configurada bem aqui, imediatamente antes de
        ...
        } catch (__overrideFinalErr) {
          try {
            fs.appendFileSync(
              path.dirname(fileURLToPath(import.meta.url)) + '/../../../logs/debug-trigger.log',
              JSON.stringify({ ts: new Date().toISOString(), marca: 'OVERRIDE_FINAL_ERRO', personality, erro: String(__overrideFinalErr && __overrideFinalErr.stack || __overrideFinalErr) }) + '\n'
            );
          } catch {}
        }
          ...
          result.resp.forEach(resposta => {
            ...
            if (resposta && typeof resposta === 'object') {
              ...
              if (resposta.resp && typeof resposta.resp === 'string' && resposta.resp.trim().length > 0) {
                ...
                if (!resposta.react) {
                  resposta.react = getNazunaReact(isNightTime, personality);
                }
              ...
              // Se a resposta tem outro formato (ex: só texto direto no objeto)
                ...
                respostas.push({
                  ...
                  react: resposta.react || getNazunaReact(isNightTime, personality)
                });
              }
            }
            // Se a resposta é uma string diretamente
              ...
              respostas.push({
                ...
                react: getNazunaReact(isNightTime, personality)
              });
            }
          });
        } 
        // Se não tem respostas válidas, tentar criar uma resposta padrão
          ...
          console.warn(`⚠️ [${personality}] Resposta da IA não tem formato esperado`, {
...
function getNazunaReact(isNightTime, personality = 'gyomei') {
  // Cada personalidade agora é distinta (não colapsa mais tudo em Gyomei),
  // então o pool de emoji também respeita quem está respondendo.
  const poolsPorPersonalidade = {
    gyomei: ['🙏', '🕊️', '🌊', '⛰️', '😌', '🤝', '🔥', '💪', '🌙', '✨'],
    nazuna: ['🌸', '🌙', '🦇', '💕', '😊', '😳', '😅', '😠', '🌟', '✨'],
    tanjiro: ['😊', '💪', '🌻', '🙏', '✨', '🔥', '😌', '🤝'],
    zenitsu: ['😭', '😳', '⚡', '🙏', '😅', '💦', '😨', '✨'],
  ...
  const reactionsNeutras = ['😊', '👍', '✨', '🙂', '💬', '🤔', '👌'];

  const chavePersonalidade = String(personality || '').trim().toLowerCase();
  const pool = poolsPorPersonalidade[chavePersonalidade] || reactionsNeutras;

  if (isNightTime) {
    return pool[Math.floor(Math.random() * Math.ceil(pool.length / 2)) + Math.floor(pool.length / 2)];
  }

  return pool[Math.floor(Math.random() * Math.ceil(pool.length / 2))];
}
...
async function funcaoNovaTesteMauricio() {
  throw new Error('FUNCAO_NOVA_TESTE_MAURICIO_' + Date.now() + '_de_verdade_e_totalmente_nova');
}

export {
  funcaoNovaTesteMauricio,
