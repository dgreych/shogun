...
const loadSubdonos = () => {
  const data = loadJsonFile(SUBDONOS_FILE, {
    subdonos: []
  });
  
  // Garante que subdonos é um array
  let subdonos = data.subdonos || [];
  if (!Array.isArray(subdonos)) {
    subdonos = [];
  }
  
  // Remove entradas nulas/undefined/inválidas automaticamente
  const cleanedSubdonos = subdonos.filter(id => id && typeof id === 'string' && id.trim().length > 0);
  
  // Se houve limpeza, salva automaticamente
  if (cleanedSubdonos.length !== subdonos.length) {
    console.log(`🔧 Limpeza automática de subdonos: ${subdonos.length - cleanedSubdonos.length} entradas inválidas removidas`);
    saveSubdonos(cleanedSubdonos);
    return cleanedSubdonos;
  }
...
const saveSubdonos = subdonoList => {
  try {
    ensureDirectoryExists(DONO_DIR);
    // Garante que é um array e remove valores inválidos
    const cleanedList = Array.isArray(subdonoList) 
      ? subdonoList.filter(id => id && typeof id === 'string' && id.trim().length > 0)
      : [];
    
    fs.writeFileSync(SUBDONOS_FILE, JSON.stringify({
      subdonos: cleanedList
    }, null, 2));
    return true;
  } catch (error) {
...
/**
 * Verifica se um usuário é subdono
 * @param {string} userId - ID do usuário a ser verificado
 * @returns {boolean} - True se for subdono, false caso contrário
 */
const isSubdono = userId => {
  // Validação inicial
  if (!userId) return false;
  
  // Garante que userId é string
  if (typeof userId !== 'string') {
    userId = String(userId);
  }
  
  // Carrega lista de subdonos (já limpa automaticamente)
  const currentSubdonos = loadSubdonos();
  
  // Se a lista estiver vazia, retorna false
  if (!currentSubdonos || currentSubdonos.length === 0) {
    return false;
  }
  
  // Remove sufixos para comparação (tanto @s.whatsapp.net quanto @lid)
  const userIdBase = userId.replace(/@s\.whatsapp\.net|@lid/g, '');
  
  // Verifica se o userId está na lista de subdonos
  return currentSubdonos.some(subdonoId => {
    // IGNORA VALORES NULOS/INVÁLIDOS - segurança extra
    if (!subdonoId || typeof subdonoId !== 'string') return false;
    
    // Comparação direta ou por base (sem sufixos)
...
/**
 * Limpa manualmente a lista de subdonos removendo entradas inválidas
 * @returns {Object} Resultado da limpeza
 */
const cleanSubdonosList = () => {
  const currentSubdonos = loadSubdonos();
  const cleaned = currentSubdonos.filter(id => id && typeof id === 'string' && id.trim().length > 0);
  const removedCount = currentSubdonos.length - cleaned.length;
  
  if (removedCount > 0) {
    saveSubdonos(cleaned);
    return {
      success: true,
      removed: removedCount,
      remaining: cleaned.length,
      message: `✅ ${removedCount} entrada(s) inválida(s) removidas da lista de subdonos.`
    };
  }
  ...
  return {
    ...
    removed: 0,
    remaining: cleaned.length,
    message: '✅ Nenhuma entrada inválida encontrada na lista de subdonos.'
  };
};
