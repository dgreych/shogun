...
import { getActivePersona } from './gyomeiCore.js';
...
// Camada por persona: se a personalidade ativa tiver uma mídia própria pra
// esse slot (setmidia <persona> <slot>), ela vence; senão cai na mídia
...
function installCommandMediaInterceptor(sock) {
  ...
  sock.sendMessage = async (jid, content, options) => {
    try {
      // Cards e outras mídias produzidas dinamicamente não podem ser trocados
      // por uma mídia estática cadastrada via setmidia. O marcador é removido
      // antes de chegar ao Baileys e serve apenas para atravessar esta camada.
      if (content?.__gyomeiPreserveDynamicMedia === true) {
        const preservedContent = { ...content };
        delete preservedContent.__gyomeiPreserveDynamicMedia;
        commandContext.delete(jid);
        return originalSendMessage(jid, preservedContent, options);
      }
      ...
      if (fresh && replaceable) {
        ...
        if (custom?.path && fs.existsSync(custom.path)) {
          ...
          // Buffer de verdade, não { url: path }. O resto do projeto já tinha
          // aprendido essa lição (upscale, rmbg, welcome card) e essa camada
          // de mídia por comando tinha ficado pra trás: o Baileys aceita
          // { url } pra imagem sem reclamar, mas pra vídeo/gif ele silenciosamente
          // não entrega nada — sem lançar erro, só some. Foi isso que deixou os
          // menus mudos (só a reação de emoji, zero conteúdo, zero erro no log).
