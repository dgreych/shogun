...
const DEFAULT_TAVERN_DATABASE_PATH = path.join(DATABASE_DIR, 'tavern.sqlite');
...
class SqliteStore {
  ...
  constructor({
    filename = process.env.TAVERN_DATABASE_PATH || DEFAULT_TAVERN_DATABASE_PATH,
    ...
    if (this.filename !== ':memory:') {
      try {
        ...
        if (stats.size > this.maxDatabaseBytes) {
          throw new Error(`Banco Tavern excede o limite de ${this.maxDatabaseBytes} bytes`);
        }
  ...
  assertOpen() {
    if (this.closed || !this.database) {
      throw new Error('Banco Tavern fechado');
    }
  }
  ...
  async applyMigrations() {
    this.assertOpen();
    const session = new SqliteSession(this.database);

    session.exec(`
      CREATE TABLE IF NOT EXISTS tavern_schema_migrations (
    ...
    const applied = new Set(
      session.all('SELECT version FROM tavern_schema_migrations').map(row => Number(row.version))
    );
  ...
  read(operation) {
    return this.enqueue(() => {
      this.assertOpen();
      return operation(new SqliteSession(this.database));
    });
  }
