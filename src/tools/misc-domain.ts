...
type DictionaryResult = {
  ...
  readonly separacao?: string;
  readonly significados: readonly string[];
  readonly exemplos: readonly { readonly texto: string; readonly fonte?: string }[];
  readonly etimologia?: string;
  readonly frases: readonly { readonly texto: string; readonly autor?: string }[];
};

type QrReader = {
  readQRCode(buffer: Buffer): Promise<{ readonly message: string }>;
};

type AsyncMediaStream = AsyncIterable<Uint8Array>;

type HttpPost = (url: string, body: unknown) => Promise<{ readonly data: Record<string, unknown> }>;

type Handler = (context: MiscToolsExecutionContext) => Promise<void>;

export interface MiscToolsExecutionContext extends MacrotrancheExecutionContext {
  readonly command?: string;
  readonly pickLoadingMessage?: (() => string) | null;
  readonly styleText?: ((text: string) => Promise<readonly string[]> | readonly string[]) | null;
  readonly qrReader?: QrReader | null;
  readonly downloadContentFromMessage?: ((message: unknown, type: 'image') => Promise<AsyncMediaStream> | AsyncMediaStream) | null;
  ...
  readonly isQuotedVideo?: boolean;
  readonly isQuotedDocument?: boolean;
  readonly isQuotedAudio?: boolean;
}
...
function socketOf(context: MiscToolsExecutionContext): LegacySocket {
...
function quotedMessageOf(context: MiscToolsExecutionContext): Record<string, unknown> | null {
...
async function nickCommand(context: MiscToolsExecutionContext): Promise<void> {
...
async function qrCodeCommand(context: MiscToolsExecutionContext): Promise<void> {
  const query = context.query.trim();
  if (!query) {
...
async function readQrCommand(context: MiscToolsExecutionContext): Promise<void> {
...
async function shortenLinkCommand(context: MiscToolsExecutionContext, httpPost: HttpPost): Promise<void> {
  const query = context.query.trim();
...
async function dictionaryCommand(context: MiscToolsExecutionContext): Promise<void> {
  const query = context.query.trim();
  ...
  try {
    ...
    if (!result || result.significados.length === 0) throw new Error('Sem resultados');
    let text = `📘✨ *Significado de "${result.palavra || word.toUpperCase()}":*\n\n`;
    if (result.classe) text += `*📚 Classe:* ${result.classe}\n\n`;
    if (result.separacao) text += `*🔤 Separação:* ${result.separacao}\n\n`;
    text += '*📖 Significados:*\n';
    result.significados.slice(0, 3).forEach((meaning, index) => { text += `${index + 1}. ${meaning}\n`; });
    if (result.significados.length > 3) text += `_...e mais ${result.significados.length - 3} significados_\n`;
    text += '\n';
    const example = result.exemplos[0];
    if (example) {
      text += `*💡 Exemplo:*\n${example.texto}\n`;
      if (example.fonte) text += `_Fonte: ${example.fonte}_\n`;
      text += '\n';
    }
    if (result.etimologia) text += `*📜 Etimologia:*\n${result.etimologia.substring(0, 150)}...\n\n`;
    const phrase = result.frases[0];
    if (phrase) {
      text += `*✨ Frase:*\n“${phrase.texto}”\n`;
      if (phrase.autor && phrase.autor !== 'Desconhecido') text += `_— ${phrase.autor}_\n`;
    }
...
async function translatorCommand(context: MiscToolsExecutionContext): Promise<void> {
  const query = context.query.trim();
...
async function uploadCommand(context: MiscToolsExecutionContext): Promise<void> {
  ...
  try {
    const quoted = quotedMessageOf(context);
    if (!quoted) throw new Error('Mensagem citada ausente.');
    const getFileBuffer = required(context.getFileBuffer, 'getFileBuffer');
    let media: unknown;
    if (context.isQuotedDocument) media = await getFileBuffer(quoted.documentMessage, 'document');
    else if (context.isQuotedVideo) media = await getFileBuffer(quoted.videoMessage, 'video');
    else if (context.isQuotedImage) media = await getFileBuffer(quoted.imageMessage, 'image');
    else media = await getFileBuffer(quoted.audioMessage, 'audio');
...
implements VNextCommandDispatchTarget<MiscToolsExecutionContext> {
  constructor(private readonly httpPost: HttpPost = (url, body) => axios.post(url, body)) {}

  async dispatch(command: string, context: MiscToolsExecutionContext): Promise<boolean> {
    const normalized = String(command || '').trim().toLowerCase();
    let handler: Handler | undefined;
    if (['nick', 'gerarnick', 'nickgenerator'].includes(normalized)) handler = nickCommand;
    else if (normalized === 'qrcode') handler = qrCodeCommand;
    else if (['lerqr', 'readqr', 'scanqr'].includes(normalized)) handler = readQrCommand;
    else if (['encurtalink', 'tinyurl'].includes(normalized)) {
      handler = (ctx) => shortenLinkCommand(ctx, this.httpPost);
    } else if (['dicionario', 'dictionary'].includes(normalized)) handler = dictionaryCommand;
    else if (['tradutor', 'translator'].includes(normalized)) handler = translatorCommand;
    else if (['upload', 'imgpralink', 'videopralink', 'gerarlink'].includes(normalized)) handler = uploadCommand;
