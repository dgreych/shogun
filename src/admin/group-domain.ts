...
async function sendX9(
  ...
  if (context.groupData.x9 !== true) return;
  try {
...
async function linkGroup(context: AdminExecutionContext): Promise<void> {
  if (!(await ensureAdminAccess(context, adminAccess({ botAdmin: true })))) return;
...
async function listRequests(context: AdminExecutionContext): Promise<void> {
  if (!(await ensureAdminAccess(context, adminAccess({ botAdmin: true })))) return;
  try {
...
async function setGroupName(context: AdminExecutionContext): Promise<void> {
  if (!(await ensureAdminAccess(context, adminAccess({ realAdmin: true, botAdmin: true })))) return;
  ...
  try {
    ...
    await sendX9(
      context,
      `✏️ *X9 Report:* Nome do grupo alterado por @${context.sender.split('@')[0]}\n\n🔹 Anterior: *${metadata.subject || 'Nome anterior'}*\n🔸 Novo: *${name}*`,
      [context.sender],
    );
...
async function setGroupDescription(context: AdminExecutionContext): Promise<void> {
  if (!(await ensureAdminAccess(context, adminAccess({ botAdmin: true })))) return;
  ...
  try {
    ...
    await sendX9(
      context,
      `📝 *X9 Report:* Descrição do grupo alterada por @${context.sender.split('@')[0]}`,
      [context.sender],
    );
...
async function setGroupMode(context: AdminExecutionContext): Promise<void> {
  if (!(await ensureAdminAccess(context, adminAccess({ botAdmin: true })))) return;
  ...
  try {
    ...
    if (open) {
      await sendX9(
        context,
        `🔓 *X9 Report:* Grupo aberto por @${context.sender.split('@')[0]}. Agora todos podem enviar mensagens.`,
        [context.sender],
      );
      ...
      await sendX9(
        context,
        `🔒 *X9 Report:* Grupo fechado por @${context.sender.split('@')[0]}. Apenas ADMs podem enviar mensagens.`,
        [context.sender],
      );
...
async function clearChat(context: AdminExecutionContext): Promise<void> {
  if (!(await ensureAdminAccess(context, adminAccess({ botAdmin: true })))) return;
  try {
...
async function resetRank(context: AdminExecutionContext): Promise<void> {
...
async function drawMembers(context: AdminExecutionContext): Promise<void> {
  if (!context.isGroup) {
  ...
  try {
    ...
    const candidates = context.groupMembers.filter(
      (member) => !['0', 'marca'].includes(String(marks[member] ?? '')),
    );
