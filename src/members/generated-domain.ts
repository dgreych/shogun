// @ts-nocheck
// AUTO-GENERATED FROM dados/src/index.js. DO NOT EDIT BY HAND.
import type { VNextCommandDispatchTarget } from '../runtime/compatibility-dispatch.js';

export type MembersGeneratedScope = Record<string, any> & { command: string };

export interface MembersGeneratedExecutionContext {
  readonly membersScope: MembersGeneratedScope;
}

type MembersGeneratedHandler = (scope: MembersGeneratedScope) => Promise<unknown>;
...
async function member_005_role_vou(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_006_role_nvou(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
  let { args, ensureRoleParticipants, groupData, groupPrefix, isGroup, persistGroupData, refreshRoleAnnouncement, reply, sanitizeRoleCode, sender } = scope;
  try {
    switch (command) {
...
async function member_007_role(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_016_conquistas(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_059_reputacao(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
  let { getEcoUser, groupData, isGroup, loadEconomy, prefix, pushname, reply, saveEconomy, sender } = scope;
  try {
    switch (command) {
...
async function member_074_slots(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_082_presente(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_086_vender(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_217_zipbot(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_218_gitbot(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_307_rankativos(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_308_rankinativos(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_309_checkativo(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_310_atividade(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_311_totalcmd(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_312_meustatus(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_314_statusbot(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_316_topcmd(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_318_statusgp(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_324_ping(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_343_mention(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_440_caixa(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_442_inventario(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_444_toprep(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_445_denunciar(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_446_denuncias(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_500_afk(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_501_voltei(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
async function member_502_regras(scope: MembersGeneratedScope): Promise<unknown> {
  const command = String(scope.command || "").trim().toLowerCase();
...
const HANDLERS = new Map<string, MembersGeneratedHandler>([
...
implements VNextCommandDispatchTarget<MembersGeneratedExecutionContext> {
  async dispatch(command: string, context: MembersGeneratedExecutionContext): Promise<boolean> {
