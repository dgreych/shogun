import { type Intencao } from './intents.js';
...
const REGRAS: ReadonlyArray<readonly [RegExp, Intencao]> = [
...
const PADRAO: Intencao = 'brincar';
export function intencaoDoComando(comando: string): Intencao {
  const t = String(comando || '').toLowerCase();
  for (const [re, intencao] of REGRAS) if (re.test(t)) return intencao;
  return PADRAO;
}
